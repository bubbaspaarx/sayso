...
## Motion suffix (append to every image-to-video prompt — Kling has no negative-prompt box, so the negatives live here)

> Very subtle ambient motion only. Slow, gentle, looping. The camera is
> completely static: no zoom, no pan, no drift. No people, no faces, no hands.
> No text or watermark. Nothing new appears, nothing changes shape, no
> morphing, no flicker. Everything else in the picture stays exactly as it is.

## Scenes

| scene id | still prompt | motion prompt |
|---|---|---|
| `bedroom-dark` | A child's bedroom at night seen from the pillow, window with curtains half open, beyond the window a dark garden and at the very bottom a small dark greenhouse with no light in it. Moonlight only. | The curtains stay exactly where they are and do not move. Only the moonlight changes, slowly brightening and dimming as thin cloud passes across the moon. Nothing else moves. |
...
| `kitchen-night` | A cottage kitchen at night lit by a single bulb, wooden table with a small torch on it, a key hanging on a hook by the window, and an old brass-and-glass lantern by the back door, glass slightly fogged. | The bare bulb sways almost imperceptibly on its cord, its light shifting very slightly on the table. Nothing else moves. No fog, no smoke, no steam. |
...
| `garden-fork` | View from a back doorstep at night: directly ahead a gravel garden path forks into two clearly separate paths in the foreground, like a letter Y. The left fork curves away toward a moonlit pond. The right fork leads toward a dark wooden shed with no lights, black windows. Cold mist. Stars. | Grass sways. Mist drifts low across both paths. Stars twinkle. |
| `shed-jackdaws` | A dark wooden garden shed at night, completely unlit, black windows, five jackdaws on the roof with wings half-spread and eyes catching the moonlight, feathers scattered in the air. Dramatic but not scary. | Feathers drift down. Wings twitch. One jackdaw's eye blinks. |
| `pond-night` | A small garden pond at night, perfectly still water reflecting a full moon, reeds at the edge, a frog on a lily pad, in the far distance a greenhouse with no light. | The frog stays perfectly still on its lily pad and does not move at all. Only the water moves: a single slow ripple spreads outward across the moon's reflection. Reeds sway very gently. |
| `greenhouse-dark` | Inside a Victorian glass greenhouse at night, all shadows and moonlit panes, and in the centre on a wooden bench a single small plant in a clay pot with its leaves drooping, grey and dull. Sad and quiet. | Moonlight through the glass slowly brightens and dims as thin cloud passes. A few tiny specks of dust glint in the light. Nothing else moves. No smoke, no mist, no wisps. |
