...
- Brand: `public/brand/` (outlined Nunito 900 wordmark "Say|so" + lowercase "so" icon; SVG/PNG sets), `public/favicon.svg`, `src/components/Wordmark.tsx`. Palette rule holds: amber for read/UI, Felix blue only for Felix. Regenerate paths with fontTools if the mark changes (see Workboard "Logo and brand assets").
